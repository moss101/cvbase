...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await logAi(user.id, { function: 'ai-linkedin', model: ROUTED_MODEL_LABEL, status: 'ok' });
    return ok(result);
  } catch (err) {
    if (userId) await logAi(userId, { function: 'ai-linkedin', model: ROUTED_MODEL_LABEL, status: 'error' });
    return fail(err);
  }
});
