...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await logAi(user.id, { function: 'ai-cover-letter', model: 'gemini-2.5-flash', status: 'ok' });
    return ok(result);
  } catch (err) {
    if (userId) await logAi(userId, { function: 'ai-cover-letter', model: 'gemini-2.5-flash', status: 'error' });
    return fail(err);
  }
});
