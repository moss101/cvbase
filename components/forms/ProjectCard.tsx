    ...
    return (
        <div className="border border-border rounded-2xl p-8 mb-5 bg-gray-50/50 transition-all hover:shadow-md hover:border-gray-300 relative pdf-item">
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-bold text-dark">{item.name || t('projectCard.newProject', 'New Project')}</h3>
            ...
            <form className="grid grid-cols-2 gap-6">
                <div>
                    <label className="font-semibold mb-2 flex items-center text-sm text-gray-700">
                        <span>{t('projects.name', 'Project Name')}</span>
                ...
                <div>
                    <label className="font-semibold mb-2 flex items-center text-sm text-gray-700">
                        <span>{t('projectCard.link', 'Link')}</span>
                ...
                <div className="col-span-2">
                    <label className="font-semibold mb-2 flex items-center text-sm text-gray-700">
                        <span>{t('projects.technologies', 'Technologies Used')}</span>
                        <AITipHelper section="projects" fieldName="Technologies Used" currentValue={item.technologies} />
                    </label>
                    <input type="text" className="w-full p-3 border border-border rounded-lg bg-white" name="technologies" placeholder={t('projectCard.technologiesPlaceholder', 'e.g., React, Node.js, PostgreSQL')} value={item.technologies} onChange={handleChange} />
                </div>
                <div>
                    <label className="font-semibold mb-2 block text-sm text-gray-700">{t('experience.startDate', 'Start Date')}</label>
                    <input type="text" className="w-full p-3 border border-border rounded-lg bg-white" name="startDate" value={item.startDate} onChange={handleChange} placeholder={t('projectCard.startDatePlaceholder', 'e.g. Jan 2021')} />
                </div>
                <div>
                    <label className="font-semibold mb-2 block text-sm text-gray-700">{t('experience.endDate', 'End Date')}</label>
                    <input type="text" className="w-full p-3 border border-border rounded-lg bg-white disabled:bg-gray-100" name="endDate" value={isCurrent ? '' : item.endDate} onChange={handleChange} disabled={isCurrent} placeholder={t('itemCard.endDatePlaceholder', 'e.g. Present')} />
                     <div className="mt-2 flex items-center">
                        ...
                        <label htmlFor={`current-project-${item.id}`} className="ml-2 block text-sm text-gray-900">{t('projectCard.ongoingProject', 'Ongoing project')}</label>
                    </div>
                </div>
                <div className="col-span-2">
                    ...
                    <textarea className="w-full p-3 border border-border rounded-lg bg-white min-h-[120px] resize-y" name="description" value={item.description} onChange={handleChange} placeholder={t('projectCard.descPlaceholder', 'Describe the project, your role, and the outcome.')} />
                </div>
            </form>
        </div>
    );
