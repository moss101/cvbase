        ...
        switch (id) {
            case 'summary':
                return summary.professionalSummary ? (
                    <section key="summary" data-section="summary">
                        <div className="break-after-avoid">{renderSectionHeader('Summary', '01')}</div>
            ...
            case 'experience':
                return experience.length > 0 ? (
                    <section key="experience" data-section="experience">
                        <div className="break-after-avoid">{renderSectionHeader('Professional Experience', '03')}</div>
            ...
            case 'education':
                return education.length > 0 ? (
                    <section key="education" data-section="education">
                        <div className="break-after-avoid">{renderSectionHeader('Academics', '05')}</div>
            ...
            case 'skills':
                return skills.length > 0 ? (
                    <section key="skills" data-section="skills">
                        <div className="break-after-avoid">{renderSectionHeader('Technical Stack', '02')}</div>
            ...
            case 'projects':
                return projects.length > 0 ? (
                    <section key="projects" data-section="projects">
                        <div className="break-after-avoid">{renderSectionHeader('Selected Repositories & Projects', '04')}</div>
                        <div className="space-y-4 pt-1">
                            {projects.map(proj => (
            ...
            case 'certifications':
                return certifications.length > 0 ? (
                    <div key="certifications" data-section="certifications" className="col-span-1">
                        <h3 className="text-[9px] font-bold font-mono uppercase tracking-wider text-slate-500 break-after-avoid">
                            Certifications
                        </h3>
                        <ul className="text-slate-650 space-y-0.5 mt-1 list-disc pl-3">
                            {certifications.map(c => (
                                <li key={c.id} className="text-[10px] break-inside-avoid">{c.name}</li>
                            ))}
                        </ul>
                    </div>
                ) : null;
                ...
                return languages.length > 0 ? (
                    <div key="languages" data-section="languages" className="col-span-1">
                        ...
                        <ul className="text-slate-655 space-y-0.5 mt-1 list-disc pl-3 font-mono">
                            {languages.map(l => (
                                <li key={l.id} className="text-[10px] break-inside-avoid">{l.language} ({l.proficiency})</li>
                            ))}
                        </ul>
                    </div>
                ) : null;
                ...
                return awards.length > 0 ? (
                    <div key="awards" data-section="awards" className="col-span-1">
                        ...
                        <ul className="text-slate-655 space-y-0.5 mt-1 list-disc pl-3">
                            {awards.map(a => (
                                <li key={a.id} className="text-[10px] break-inside-avoid">{a.title}</li>
                            ))}
                        </ul>
                    </div>
                ) : null;
        ...
        >
            ...
            <div>
                ...
                <div className="grid grid-cols-3 gap-4 pt-2">
                    {renderRun(['certifications', 'languages', 'awards'])}
                </div>
            </div>
        </div>
    );
...
export default React.memo(EecsMitTemplate);
