...
const CleanTemplate: React.FC<ResumePreviewProps> = ({ formData, isCardPreview, visibleSections, settings }) => {
    ...
    return (
        <div id={isCardPreview ? undefined : "resume-preview-clean"} className={`w-[794px] min-h-[1123px] h-auto bg-white text-[#333] p-12 ${fontSize} font-sans`} style={{ fontFamily: settings?.fontFamily || 'Arial, sans-serif' }}>
            ...
            {experience.length > 0 && (
                <section className="mb-8">
                    <h2 className="text-sm font-bold uppercase tracking-wider mb-4" style={{ color: themeColor }}>Professional Experience</h2>
            ...
            {skills.length > 0 && (
                <section className="mb-8">
                    <h2 className="text-sm font-bold uppercase tracking-wider mb-3" style={{ color: themeColor }}>Skills</h2>
            ...
            {/* Simplified rendering for other sections */}
            {[
                ...
                { id: 'volunteer', title: 'Volunteering', data: volunteer }
                 ...
                 visibleSections.includes(section.id as any) && section.data && section.data.length > 0 && (
                    <section key={section.id} className="mb-8">
                        <h2 className="text-sm font-bold uppercase tracking-wider mb-3" style={{ color: themeColor }}>{section.title}</h2>
