...
export const MemoryMenu: React.FC<MemoryMenuProps> = ({ conversation, busy = false, onRename, onArchive, onReopen, onExport, onForgetSummary, onStartSelect, onDelete }) => {
    ...
    const item = (label: string, icon: React.ReactNode, onClick: () => void, opts: { danger?: boolean; disabled?: boolean } = {}) => (
        <li>
            <button
                ...
                className={`tap-target flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left text-sm transition-colors hover:bg-surface-canvas focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus-ring disabled:opacity-50 ${opts.danger ? 'text-status-danger' : 'text-content-primary'}`}
            >
    ...
    return (
        <>
            <Button variant="secondary" size="sm" icon={<MoreHorizontal size={14} strokeWidth={2} />} onClick={() => setOpen(true)} aria-haspopup="dialog" aria-expanded={open}>
                {t('careeros.coach.memory', 'Memory')}
            </Button>
