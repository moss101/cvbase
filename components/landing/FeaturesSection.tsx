            ...
            <div className="mx-auto max-w-7xl px-5 sm:px-8">
                ...
                >
                    <p className="font-label text-[0.7rem] tracking-[0.24em] uppercase text-ink-faint">
                        02 <span className="text-ember">—</span> {t('landing.features.eyebrow', 'The toolkit')}
                    </p>
                    <h2 className="mt-5 font-display font-medium tracking-[-0.015em] text-ink text-4xl sm:text-5xl leading-[1.08] [text-wrap:balance]">
                        {t('landing.features.heading.prefix', 'Every tool between a blank page and a')} <span className="italic text-ember">{t('landing.features.heading.highlight', 'signed offer')}</span>.
                    </h2>
                </motion.div>
                ...
                <div className="grid lg:grid-cols-12 gap-5 lg:gap-6">
                            ...
                            <div className="flex-1 mb-7 rounded-2xl bg-paper border border-ink/[0.06] p-5 sm:p-6">
                                ...
                                <div className="rounded-xl bg-paper-bright border border-ink/[0.08] p-4">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="font-label text-[0.6rem] tracking-[0.18em] uppercase text-ink-faint">{t('landing.features.aiEditor.draftLabel', 'Your draft')}</span>
                                        <span className="px-2 py-0.5 rounded-md bg-ink/[0.05] text-ink-faint text-[0.65rem] font-semibold">{t('landing.features.aiEditor.weakBadge', 'Weak')}</span>
                                    </div>
                                    <p className="text-[0.9rem] text-ink-faint line-through decoration-ink/20">
                                        {t('landing.features.aiEditor.beforeText', "Responsible for managing the company's social media accounts.")}
                                    </p>
                                </div>
                                ...
                                <div className="flex items-center justify-center gap-2.5 py-3">
                                    ...
                                    <span className="font-label text-[0.66rem] tracking-[0.18em] uppercase text-ink-soft">{t('landing.features.aiEditor.rewriteLabel', 'AI rewrite')}</span>
                                ...
                                <div className="rounded-xl bg-paper-bright border-2 border-ember/60 p-4 shadow-[0_14px_30px_-12px_rgba(200,68,44,0.3)]">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="font-label text-[0.6rem] tracking-[0.18em] uppercase text-ember">{t('landing.features.aiEditor.suggestedLabel', 'Suggested')}</span>
                                        <span className="px-2 py-0.5 rounded-md bg-ember-tint text-ember-deep text-[0.65rem] font-semibold">{t('landing.features.aiEditor.strongBadge', '✓ Strong')}</span>
                                    </div>
                                    <p className="text-[0.9rem] text-ink font-medium">
                                        {t('landing.features.aiEditor.afterText', 'Grew engagement 340% across 5 channels, converting 12k followers into a qualified lead pipeline worth $86k/quarter.')}
                                    </p>
                                    <div className="flex gap-2 mt-3.5">
                                        <span className="px-3.5 py-1.5 rounded-full bg-ink text-paper text-[0.72rem] font-semibold cursor-default">{t('landing.features.aiEditor.acceptButton', 'Accept')}</span>
                                        <span className="px-3.5 py-1.5 rounded-full bg-ink/[0.05] text-ink-soft text-[0.72rem] font-semibold cursor-default">{t('landing.features.aiEditor.tryAnotherButton', 'Try another')}</span>
                                    </div>
                                </div>
                            </div>
                            <h3 className="font-display text-2xl font-semibold text-ink">{t('landing.features.aiEditor.title', 'The AI editor')}</h3>
                            <p className="mt-2.5 text-[0.97rem] leading-relaxed text-ink-soft max-w-lg">
                                {t('landing.features.aiEditor.description', 'It rewrites vague duties into quantified achievements — the language recruiters shortlist. Every suggestion is yours to accept, sharpen, or ignore.')}
                            </p>
                        </div>
                    </motion.article>
                    ...
                    <motion.article {...cardReveal(0.1)} className={`${cardClass} lg:col-span-5`}>
                        <div className={innerClass}>
                            <div className="flex-1 mb-7 rounded-2xl bg-paper border border-ink/[0.06] p-5 flex flex-col items-center justify-center gap-5">
                                <div className="relative w-28 h-28">
                                    ...
                                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                                        ...
                                        <span className="font-label text-[0.55rem] tracking-[0.2em] uppercase text-ink-faint mt-0.5">{t('landing.features.atsChecker.scoreLabel', 'ATS Score')}</span>
                                    </div>
                                </div>
                                <ul className="w-full max-w-[230px] space-y-2.5">
                                    {[
                                        t('landing.features.atsChecker.check1', 'Keywords matched'),
                                        t('landing.features.atsChecker.check2', 'Clean structure'),
                                        t('landing.features.atsChecker.check3', 'Parseable fonts'),
                                    ].map((check) => (
                            ...
                            <h3 className="font-display text-2xl font-semibold text-ink">{t('landing.features.atsChecker.title', 'The ATS checker')}</h3>
                            <p className="mt-2.5 text-[0.97rem] leading-relaxed text-ink-soft">
                                {t('landing.features.atsChecker.description', 'Score your CV against tracking systems before you apply — and fix every red flag in one click.')}
                            </p>
                        </div>
                    </motion.article>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div className="rounded-lg bg-paper-bright border border-ink/[0.08] overflow-hidden relative group-hover:-translate-y-1 transition-transform duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] delay-150">
                                        ...
                                        <div className="absolute inset-0 grid place-items-center bg-ink/90">
                                            <span className="text-paper font-label text-[0.7rem] tracking-[0.1em]">{t('landing.features.templates.moreBadge', '+36 more')}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <h3 className="font-display text-2xl font-semibold text-ink">{t('landing.features.templates.title', 'The templates')}</h3>
                            <p className="mt-2.5 text-[0.97rem] leading-relaxed text-ink-soft">
                                {t('landing.features.templates.description', 'Industry-specific layouts, typeset like fine print. Switch designs anytime — your content carries over untouched.')}
                            </p>
                        </div>
                    </motion.article>
                            ...
                            <div className="flex-1 mb-7 rounded-2xl bg-paper border border-ink/[0.06] p-5 sm:p-6 flex flex-col justify-center gap-3">
                                <div className="flex items-center gap-3.5 rounded-xl bg-paper-bright border border-ink/[0.08] px-4 py-3.5">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="font-label text-[0.62rem] tracking-[0.08em] text-ink-faint mt-0.5">{t('landing.features.export.fileSpec', 'A4 · 184 KB · print-ready')}</p>
                                    </div>
                                ...
                                <div className="flex items-center gap-3 rounded-xl bg-paper-bright border border-ink/[0.08] px-4 py-3.5">
                                    ...
                                    <span className="px-3 py-1.5 rounded-full bg-ink/[0.05] text-ink text-[0.7rem] font-bold shrink-0">{t('landing.features.export.copyLinkButton', 'Copy link')}</span>
                                </div>
                                <div className="flex items-center justify-center gap-1.5 font-label text-[0.66rem] tracking-[0.12em] uppercase text-ember-deep">
                                    ...
                                    {t('landing.features.export.exportedStatus', 'Exported in 1.2s — no watermark')}
                                </div>
                            </div>
                            <h3 className="font-display text-2xl font-semibold text-ink">{t('landing.features.export.title', 'The export')}</h3>
                            <p className="mt-2.5 text-[0.97rem] leading-relaxed text-ink-soft max-w-lg">
                                {t('landing.features.export.description', 'A pixel-perfect PDF, or a live link that updates itself every time you edit. What you see is exactly what the recruiter gets.')}
                            </p>
                        </div>
                    </motion.article>
                    ...
                    <motion.article {...cardReveal(0.05)} className={`${cardClass} lg:col-span-12`}>
                        <div className={`${innerClass} lg:flex-row lg:items-center lg:gap-12`}>
                            <div className="lg:max-w-md">
                                <h3 className="font-display text-2xl font-semibold text-ink">{t('landing.features.customization.title', 'The fine-tuning')}</h3>
                                <p className="mt-2.5 text-[0.97rem] leading-relaxed text-ink-soft">
                                    {t('landing.features.customization.description', 'Colors, typefaces, spacing, section order — every detail adjustable, every change previewed live. Your CV, your typography.')}
                                </p>
                            </div>
                            <div className="flex-1 mt-7 lg:mt-0 rounded-2xl bg-paper border border-ink/[0.06] p-5 sm:p-6 grid sm:grid-cols-3 gap-6">
                                <div>
                                    <p className="font-label text-[0.6rem] tracking-[0.18em] uppercase text-ink-faint mb-3">{t('landing.features.customization.accentColorLabel', 'Accent color')}</p>
                                ...
                                <div>
                                    <p className="font-label text-[0.6rem] tracking-[0.18em] uppercase text-ink-faint mb-3">{t('landing.features.customization.typefaceLabel', 'Typeface')}</p>
                                ...
                                <div>
                                    <div className="flex items-center justify-between mb-3">
                                        <p className="font-label text-[0.6rem] tracking-[0.18em] uppercase text-ink-faint">{t('landing.features.customization.spacingLabel', 'Spacing')}</p>
