...
const STATS: Stat[] = [
    { target: 94, suffix: '%', label: 'average ATS pass rate' },
    { target: 8, prefix: '~', suffix: ' min', label: 'to a finished first draft' },
    { target: 40, suffix: '+', label: 'professional templates' },
    { target: 100, suffix: '%', label: 'private & encrypted' },
];
                ...
                <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-start">
                    ...
                    >
                        <p className="font-label text-[0.7rem] tracking-[0.24em] uppercase text-ink-faint">
                            04 <span className="text-ember">—</span> Why CVbase
                        </p>
                        <h2 className="mt-5 font-display font-medium tracking-[-0.015em] text-ink text-4xl sm:text-5xl leading-[1.08] [text-wrap:balance]">
                            Past the robots. In front of <span className="italic text-ember">the humans</span>.
                        </h2>
                        <p className="mt-6 text-lg text-ink-soft leading-relaxed max-w-xl">
                            Most CVs are rejected by software before a recruiter ever reads them. CVbase is
                            engineered for both audiences — machine-readable structure underneath, beautiful
                            typesetting on top.
                        </p>
                            ...
                            {REASONS.map((reason, i) => (
                                ...
                                >
                                    ...
                                    <div>
                                        ...
                                        <p className="mt-0.5 text-[0.93rem] text-ink-soft leading-relaxed">{reason.description}</p>
                                    </div>
                                </motion.li>
                            ))}
                        </ul>
                    ...
                    >
                        ...
                        <div className="rounded-[1.75rem] bg-ink/[0.04] ring-1 ring-ink/[0.07] p-2">
                            <div className="rounded-[calc(1.75rem-0.5rem)] bg-paper-bright shadow-[inset_0_1px_1px_rgba(255,255,255,0.9)] overflow-hidden">
                                <div className="px-7 pt-6 pb-4 border-b border-ink/[0.07] flex items-center justify-between">
                                    <span className="font-label text-[0.62rem] tracking-[0.22em] uppercase text-ink-faint">By the numbers</span>
                                ...
                                <div className="grid grid-cols-2">
                                    {STATS.map((stat, i) => (
                                        ...
                                        >
                                            <CountUp stat={stat} />
                                            <p className="mt-2 text-[0.85rem] text-ink-soft leading-snug">{stat.label}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        <div className="mt-6 rounded-[1.75rem] bg-ink p-8 sm:p-9 shadow-[0_28px_60px_-18px_rgba(27,23,19,0.5)] relative overflow-hidden">
                            ...
                            <blockquote className="relative font-display text-paper text-xl sm:text-[1.35rem] leading-snug font-light italic">
                                I sent the same experience on a new CVbase template and started getting
                                callbacks within a week. The AI suggestions made my bullet points actually
                                sound impressive.
                            </blockquote>
                            <div className="relative mt-6 flex items-center justify-between">
                                <p className="text-[0.88rem] font-semibold text-paper">
                                    Daniel K. <span className="font-normal text-paper/50">— hired as Data Analyst</span>
                                </p>
                                <div className="flex gap-1 text-ember" aria-label="5 out of 5 stars">
