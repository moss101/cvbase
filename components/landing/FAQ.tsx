                    ...
                    >
                        <div className="lg:sticky lg:top-32">
                            <p className="font-label text-[0.7rem] tracking-[0.24em] uppercase text-ink-faint">
                                06 <span className="text-ember">—</span> {t('landing.faq.eyebrow', 'Fine print')}
                            </p>
                            <h2 className="mt-5 font-display font-medium tracking-[-0.015em] text-ink text-4xl sm:text-5xl leading-[1.08]">
                                {t('landing.faq.heading.prefix', 'Questions,')} <span className="italic text-ember">{t('landing.faq.heading.highlight', 'answered')}</span>.
                            </h2>
                            <p className="mt-6 text-ink-soft leading-relaxed max-w-sm">
                                {t('landing.faq.intro', 'Everything worth knowing before you hit “Create my CV”. Nothing hidden in an accordion.')}
                            </p>
                            <p className="mt-8 text-[0.95rem] text-ink-soft">
                                {t('landing.faq.contactPrompt', 'Anything else?')}{' '}
                                <a href="mailto:support@cvbase.io" className="text-ink font-semibold border-b border-ink/25 pb-0.5 hover:text-ember hover:border-ember transition-colors duration-300">
                                    {t('landing.faq.contactLink', 'Talk to our team')}
                                </a>
                            </p>
                        </div>
                    </motion.div>
                    ...
                    <div className="lg:col-span-8 sm:columns-2 gap-10">
                        {FAQ_ITEMS.map((item, i) => (
                            ...
                            >
                                ...
                                <h3 className="font-display text-lg font-semibold text-ink leading-snug">{t(`landing.faq.items.${item.key}.q`, item.q)}</h3>
                                <p className="mt-2.5 text-[0.93rem] text-ink-soft leading-relaxed">{t(`landing.faq.items.${item.key}.a`, item.a)}</p>
                            </motion.div>
                        ))}
                    </div>
                </div>
