...
const FEATURED: { id: TemplateId; badge: string; badgeStyle: 'plain' | 'ember'; Component: React.FC<any> }[] = [
    { id: 'gsb-executive', badge: 'Executive', badgeStyle: 'plain', Component: GsbExecutiveTemplate },
    { id: 'modern', badge: 'Most chosen', badgeStyle: 'ember', Component: ModernTemplate },
    { id: 'creative', badge: 'Creative', badgeStyle: 'plain', Component: CreativeTemplate },
];
                ...
                >
                    <div className="max-w-2xl">
                        <p className="font-label text-[0.7rem] tracking-[0.24em] uppercase text-ink-faint">
                            01 <span className="text-ember">—</span> Templates
                        </p>
                        <h2 className="mt-5 font-display font-medium tracking-[-0.015em] text-ink text-4xl sm:text-5xl leading-[1.08] [text-wrap:balance]">
                            {templateCount}+ ways to look brilliant <span className="italic text-ember">on paper</span>.
                        </h2>
                    </div>
                    <p className="max-w-sm text-ink-soft leading-relaxed lg:text-right lg:pb-1.5">
                        From boardroom-classic to portfolio-bold — every layout recruiter-approved
                        and ATS-tested before it ships.
                    </p>
                </motion.div>

                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-7">
                    {FEATURED.map(({ id, badge, badgeStyle, Component }, index) => (
                        <motion.div
                            key={id}
