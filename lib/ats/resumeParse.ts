...
import { extractSkills, type NormalizedSkill } from './skillTaxonomy.ts';
...
// Browser-only file text extraction (pdfjs/mammoth + the File API) lives in
// services/resumeParser.ts, which re-exports everything here. Keeping it out of
// this module makes the module runtime-neutral (browser + Deno) so the
// ats-analyze Edge Function can run the same parsing server-side.
